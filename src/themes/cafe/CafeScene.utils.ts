...
import { DEFAULT_BOARD_NAME } from './CafeScene.constants'
import type { CafeSceneRefs, SteamSprite } from './CafeScene.types'

/** Draw the shop-name board onto a canvas (used as a texture). */
export function drawBoard(canvas: HTMLCanvasElement, name: string): void {
  ...
  const text = name && name.trim() ? name.trim() : DEFAULT_BOARD_NAME
  let size = 120
...
/** Build the static café scene and return the handles the animation loop needs. */
export function buildScene(): CafeSceneRefs {
  const scene = new THREE.Scene()
  scene.background = new THREE.Color('#1d130d')
  scene.fog = new THREE.Fog('#1d130d', 9, 18)

  const wood = (color: string, rough = 0.85) =>
  ...
  const panel = new THREE.Mesh(
    ...
    [boardSide, boardSide, boardSide, boardSide, boardFaceMaterial, boardSide],
  )
  board.add(panel)
  ...
  const steamSprites: SteamSprite[] = []
  for (let i = 0; i < 3; i++) {
  ...
  const steamCup = cups[0]
  if (steamCup) steamGroup.position.copy(steamCup.position)
  scene.add(steamGroup)

  return { scene, boardCanvas, boardTexture, panel, board, boardFaceMaterial, bulbs, steamSprites }
}
