...
- **API seams** — `executeApi.ts` (run), `submissionApi.ts` (submit for code + predict, plus `fetchSubmissionHistory`), `studentApi.ts` (upsert student before submit), `sessionApi.ts` (session/room/timer — `startTimer` is scoped to one `assignmentId`, plus `endSession` and `fetchTodayLatestSession`), `assignmentSetApi.ts` (assignment sets), `solutionApi.ts` (`getAssignmentSolution` — reveal an assignment's sample solution; the endpoint is gate-free by design, so all "when can this student see it" logic lives in `StudentView.hooks.ts`, not here). Plus `identity.ts` (anon student id) and `teacherAuth.ts` (sessionStorage flag). The wire contract uses Assignment naming end to end (`/api/assignmentsets`, `assignmentId`, `assignmentSetId`) — keep these seams in lockstep with the api repo's `CONTRACT.md`. `fetchSubmissionHistory` calls an endpoint the backend hasn't built yet (CONTRACT.md S5) — it degrades to an empty result on any failure instead of throwing, by design. `fetchTodayLatestSession` and `getAssignmentSolution` degrade the same way (`null`/`{available:false}`), but their underlying endpoints *are* implemented.
