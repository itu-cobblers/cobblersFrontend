...
- **`assignments.ts`** — `export const ASSIGNMENTS: Assignment[]`, the legacy local bundle (live data comes from `assignmentSetApi.ts`), plus `export const defaultStarter`. `id` is the array index — keep them sequential from 0. Entries may carry `lesson: LessonBlock[]` (teaching text + example code shown above the task). **Passing criteria are self-contained per assignment** via `check(result)` (the "assignment boundary"): `result = { code, output, stderr, exitCode }`, return a `Verdict` `{ passed, signals?, message? }` — every failure path should carry a beginner-friendly `message` (it renders in the FeedbackBanner). Structural code checks must run on `stripCode(code)`, never raw editor text, so comments/strings can't fake a pass; `printlnArgs`/`callArgs` inspect print and call arguments. `useAssignments` calls the active assignment's `check(...)` generically and never hardcodes assignment logic. `signals` is a free-form, **theme-agnostic** bag broadcast on success (e.g. `{ studentName }`). An assignment with no `check` simply never auto-completes (true of all API-served assignments — grading is moving server-side).
